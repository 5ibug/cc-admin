...
//        Content lastContent = messageDtoList.get(messageDtoList.size() - 1).getContent();
...
//                        byte[] imageData = new ClassPathResource("/test.png").getContentAsByteArray();
        ...
        // 开启联网搜索
        return chatClient.prompt().options(DashScopeChatOptions.builder().withEnableSearch(true).build()).messages(messages).stream().chatResponse();
    }

    // 测试function + 格式化返回
    @GetMapping(value = "/test/ai/function",produces = MediaType.TEXT_EVENT_STREAM_VALUE)
    public Flux<ChatResponse> func(@RequestParam(value = "message", defaultValue = "你好") String message) throws IOException {
        var converter = new BeanOutputConverter<>(new ParameterizedTypeReference<MessageEntity>() { });

        return chatClient.prompt().functions("editUserNameFunction").user(u->u.text(message + "{format}").param("format",converter.getFormat()) ).stream().chatResponse();
    }
    ...
    public Flux<ChatResponse> test() throws IOException {
        ...
        return chatClient.prompt().messages(  List.of(
                systemMessage,
                ...
                assistantMessage
        )).user("他是什么颜色的" ).stream().chatResponse();
    }
}
